...
def optimize_skopt(params, param_names, examples, targets, splits=3, scoring_func='roc_auc'):
    ...
    model = GradientBoostingClassifier(
        loss='deviance', random_state=42, **params)
...
output_path_file = config.XGB_OUTPUT_PATH
...
if __name__ == "__main__":
    ...
    gbt_model = GradientBoostingClassifier(
        loss='deviance', random_state=42, **best_param_dict)
    print(gbt_model)

    # train on whole training set
    print("Training the final model...")
    gbt_model.fit(train_predictors[predictor_columns], train_targets)

    print("Saving model...")
    joblib.dump(
        gbt_model, '/data/ScikitModels/RoadTemp/current/GBT_ProbSR_model.pkl')
    ...
    input_predictions = gbt_model.predict_proba(
    ...
    joblib.dump(isotonic_model,
                '/data/ScikitModels/RoadTemp/current/GBT_ProbSR_model_Isotonic.pkl')
    ...
    raw_probs = gbt_model.predict_proba(
